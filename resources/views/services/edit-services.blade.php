...
<div class="max-w-xl mx-auto mt-10 p-6 bg-white shadow-md rounded-2xl">
    <h2 class="text-3xl font-bold mb-6 text-gray-800">Edit Service</h2>

    <form method="POST" action="{{ url('services/' . $service->service_id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="text" id="service_name" name="service_name" value="{{ old('service_name', $service->service_name) }}"
        ...
        <div class="mb-4">
            ...
            <input type="number" id="estimated_duration" name="estimated_duration" value="{{ old('estimated_duration', $service->estimated_duration) }}"
                class="mt-1 block w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm p-2"
                required>
        </div>
            ...
            <select id="service_type_id" name="service_type_id"
                ...
                @foreach ($serviceTypes as $type)
                    <option value="{{ $type->service_type_id }}"
                        {{ $service->service_type_id == $type->service_type_id ? 'selected' : '' }}>
                        {{ $type->type_name }}
                    </option>
                @endforeach
            </select>
            ...
            <select id="outlet_id" name="outlet_id"
                ...
                @foreach ($outlets as $outlet)
                    <option value="{{ $outlet->outlet_id }}"
                        {{ $service->outlet_id == $outlet->outlet_id ? 'selected' : '' }}>
                        {{ $outlet->outlet_name }}
                    </option>
                @endforeach
            </select>
