...
<div class="max-w-xl mx-auto mt-10 p-6 bg-white shadow-md rounded-2xl">
    <h2 class="text-3xl font-bold mb-6 text-gray-800">Add Notification Type</h2>

    <form method="POST" action="/notification-types">
        @csrf
        ...
        <div class="mb-4">
            ...
            <textarea id="template_text" name="template_text" rows="4"
                      class="mt-1 block w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm p-2 resize-none"
            >{{ old('template_text') }}</textarea>
            @error('template_text')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
