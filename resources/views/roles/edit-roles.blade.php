...
<div class="max-w-xl mx-auto mt-10 p-6 bg-white shadow-md rounded-2xl">
    ...
    <form method="POST" action="{{ url('roles/' . $role->role_id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="3"
                      class="mt-1 block w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm p-2 resize-none">{{ old('description', $role->description) }}</textarea>
            @error('description')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
