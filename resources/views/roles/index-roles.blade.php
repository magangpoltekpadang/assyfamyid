...
@section('content')
<div x-data="roleData()" x-init="init()" class="space-y-6 p-6">
    ...
    <div class="flex items-end justify-between space-x-4">
      <div class="flex-1 max-w-md">
        ...
        <input type="text" id="search" x-model="search" placeholder="Type to search..."
               ...
               @keydown.enter.prevent="fetchRoles()">
      </div>
      <div class="flex space-x-2">
        <button @click="fetchRoles()" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <template x-for="role in roles" :key="role.role_id">
            <tr>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="role.role_id"></td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="role.role_name"></td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-4">
                <a :href="`/roles/${role.role_id}`" class="text-blue-600 hover:text-blue-900" title="View">
                  <i class="fas fa-info-circle"></i>
                </a>
                <a :href="`/roles/${role.role_id}/edit`" class="text-yellow-500 hover:text-yellow-700" title="Edit">
                  <i class="fas fa-edit"></i>
                </a>
                <button @click="confirmDelete(role.role_id)" class="text-red-600 hover:text-red-900" title="Delete">
...
@push('scripts')
<script src="{{ asset('JS/roles/role-script.js') }}"></script>
@endpush
@endsection
