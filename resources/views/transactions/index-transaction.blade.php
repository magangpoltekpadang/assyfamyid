...
<div x-data="transactionData()" x-init="init()" class="space-y-6 p-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="bg-white rounded-lg shadow p-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-3xl font-semibold text-gray-900" x-text="totalTransactions"></p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm font-medium text-gray-500">Paid Transactions</p>
          <p class="mt-1 text-3xl font-semibold text-gray-900" x-text="paidTransactions"></p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm font-medium text-gray-500">Unpaid Transactions</p>
          <p class="mt-1 text-3xl font-semibold text-gray-900" x-text="unpaidTransactions"></p>
        </div>
  ...
  <div class="bg-white rounded-lg shadow p-4">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        <label for="search" class="block text-sm font-medium text-gray-700">Search Transaction</label>
        <input type="text" id="search" x-model="search" placeholder="Search code or customer..."
               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
               @keydown.enter.prevent="fetchTransactions()">
      </div>

      <div>
        <label for="payment_status_id" class="block text-sm font-medium text-gray-700">Payment Status</label>
        <select id="payment_status_id" x-model="payment_status_id"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
  ...
  >
    <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
      ...
      <div class="flex justify-end space-x-3">
        ...
        <button @click="deleteTransaction()" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Delete</button>
      </div>
    </div>
  </div>
...
@push('scripts')
<script src="{{ asset('JS/transactions/transaction-script.js') }}"></script>
@endpush
@endsection
